...
public class EnemyR6 : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (!muerto)
        {
            ...
            if (target != initialPosition && distance < attackRadius)
            {
                ...
                gameObject.GetComponent<Animator>().SetBool("diparar", true);
            ...
            else
            {
                ...
                gameObject.GetComponent<Animator>().SetBool("diparar", false);
    ...
    public void Disparo()
    {
        for (int i = 0; i < 4; i++)
        {
            Instantiate(ShotGunBullet, shotpos.transform.position, shotpos.transform.rotation);
        }
    }
